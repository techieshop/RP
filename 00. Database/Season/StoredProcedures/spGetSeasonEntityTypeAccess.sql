CREATE PROCEDURE [acc].[spGetSeasonEntityTypeAccess]
	@UserId INT,
...
AS
BEGIN
	DECLARE @SeasonEntityTypeId INT = 5;

	DECLARE @UserEntityTypeAuth [udt].[UserEntityTypeAuth]
	INSERT INTO @UserEntityTypeAuth
	SELECT * FROM [acc].[fnGetUserEntityTypeAuthSingle](@UserId, @ContextOrganizationId, @SeasonEntityTypeId, @EntityStateIds, @IsActiveOnly);
	...
	FROM @OrganizationIds [OI]
		...
		INNER JOIN [plt].[EntityOrganization] [EO] WITH(NOLOCK) ON [EO].[OrganizationId] = [OR].[OrganizationId] AND [EO].[EntityTypeId] = @SeasonEntityTypeId
