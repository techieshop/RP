CREATE PROCEDURE [acc].[spGetUserEntityTypeAccess]
	@UserId INT,
...
AS
BEGIN
	DECLARE @UserEntityTypeId INT = 1;

	DECLARE @UserEntityTypeAuth [udt].[UserEntityTypeAuth]
	INSERT INTO @UserEntityTypeAuth
	SELECT * FROM [acc].[fnGetUserEntityTypeAuthSingle](@UserId, @ContextOrganizationId, @UserEntityTypeId, @EntityStateIds, @IsActiveOnly);
	...
	FROM @OrganizationIds [OI]
		...
		INNER JOIN [plt].[EntityOrganization] [EO] WITH(NOLOCK) ON [EO].[OrganizationId] = [OR].[OrganizationId] AND [EO].[EntityTypeId] = @UserEntityTypeId
