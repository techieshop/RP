CREATE PROCEDURE [acc].[spGetMemberEntityTypeAccess]
	@UserId INT,
...
AS
BEGIN
	DECLARE @MemberEntityTypeId INT = 3;

	DECLARE @UserEntityTypeAuth [udt].[UserEntityTypeAuth]
	INSERT INTO @UserEntityTypeAuth
	SELECT * FROM [acc].[fnGetUserEntityTypeAuthSingle](@UserId, @ContextOrganizationId, @MemberEntityTypeId, @EntityStateIds, @IsActiveOnly);
	...
	FROM @OrganizationIds [OI]
		...
		INNER JOIN [plt].[EntityOrganization] [EO] WITH(NOLOCK) ON [EO].[OrganizationId] = [OR].[OrganizationId] AND [EO].[EntityTypeId] = @MemberEntityTypeId
