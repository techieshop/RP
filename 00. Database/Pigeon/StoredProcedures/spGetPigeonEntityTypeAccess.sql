CREATE PROCEDURE [acc].[spGetPigeonEntityTypeAccess]
	@UserId INT,
...
AS
BEGIN
	DECLARE @PigeonEntityTypeId INT = 4;

	DECLARE @UserEntityTypeAuth [udt].[UserEntityTypeAuth]
	INSERT INTO @UserEntityTypeAuth
	SELECT * FROM [acc].[fnGetUserEntityTypeAuthSingle](@UserId, @ContextOrganizationId, @PigeonEntityTypeId, @EntityStateIds, @IsActiveOnly);
	...
	FROM @OrganizationIds [OI]
		...
		INNER JOIN [plt].[EntityOrganization] [EO] WITH(NOLOCK) ON [EO].[OrganizationId] = [OR].[OrganizationId] AND [EO].[EntityTypeId] = @PigeonEntityTypeId
