CREATE PROCEDURE [acc].[spGetRaceEntityTypeAccess]
	@UserId INT,
...
AS
BEGIN
	DECLARE @RaceEntityTypeId INT = 6;

	DECLARE @UserEntityTypeAuth [udt].[UserEntityTypeAuth]
	INSERT INTO @UserEntityTypeAuth
	SELECT * FROM [acc].[fnGetUserEntityTypeAuthSingle](@UserId, @ContextOrganizationId, @RaceEntityTypeId, @EntityStateIds, @IsActiveOnly);
	...
	FROM @OrganizationIds [OI]
		...
		INNER JOIN [plt].[EntityOrganization] [EO] WITH(NOLOCK) ON [EO].[OrganizationId] = [OR].[OrganizationId] AND [EO].[EntityTypeId] = @RaceEntityTypeId
